...
% \title{A Novel Alignment Approach based on Information Fusion View: \\ Direct Preference Optimization with Dynamic Learnable $\beta$}
% \title{Dynamic-$\beta$ DPO: \\ A Novel Information Fusion Framework for Preference Alignment}
\title{Information Fusion Perspective on Direct Preference Optimization: A Learnable $\beta(x)$ Approach}
...
\begin{abstract}
Direct Preference Optimization (DPO) has emerged as a compelling approach for aligning Large Language Models (LLMs) with human preferences. However, the current DPO framework lacks a theoretical understanding from an information fusion perspective, and its performance is sensitive to the static trade-off parameter $\beta$. In this work, we first establish a novel theoretical framework that views DPO as an information fusion process between the reference model and human preference signals, providing insights into how different fusion operators can be utilized for preference learning. Based on this view, we propose to make $\beta$ learnable and dynamically adjusted during training, which allows the model to automatically balance between preserving reference knowledge and incorporating human feedback according to the characteristics of input instances. Through extensive experiments on various models and datasets, we demonstrate that our approach not only achieves superior performance compared to standard DPO but also provides better theoretical interpretability and training stability. The code is available at \url{https://github.com/1587causalai/info-fusion-dpo}.
\end{abstract}

\input{chapters/1_intro.tex}
\input{chapters/2_related_work.tex}
\input{chapters/3_preliminary.tex}
\input{chapters/4_work.tex}
\input{chapters/5_exp.tex}
...
\item {\bf Open access to data and code}
    ...
    \item[] Answer: \answerYes{} % Replace by \answerYes{}, \answerNo{}, or \answerNA{}.
    \item[] Justification: The code is available at \url{https://github.com/1587causalai/info-fusion-dpo}.
    \item[] Guidelines:
    \begin{itemize}
